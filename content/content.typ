...
The design decision we kept coming back to was making tags the primary organization unit, as opposed to folders or albums. It sounds simple in theory, but a natural consequence of this is that any image can belong to multiple logical groupings simultaneously, which is something a folder hierarchy just cannot do cleanly. A user would be able to apply tags like "client:Acme", "location:SaltLake", and "format:print" to a batch of images when they upload it, and then later retrieve exactly that subset of their collection by searching across any combination of those tags. This model would also scale much more naturally to large or complex libraries than folder structures could ever possibly do, since a single image is able to exist in multiple categories without needing to be duplicated or moved across directories.
...
The Select button was motivated by a pretty specific workflow, which is that a user who just bulk-uploaded a large amount of photos from a shoot would need to be able to tag all of them at once without opening each image individually. Clicking Select drops a tag panel onto the right side of the screen and lets you check off images to build a selection, then apply or remove tags across all of them in one action. This workflow also enables users to be able to run various plugins across a selection of images, which can be extremely helpful for similar reasons.
...
The challenges we ran into were mostly technical rather than interpersonal. The most recurring difficulty was infrastructure compatibility, with versioning conflicts between libraries, packages that lacked support for the specific functionality we needed, and interactions between different parts of the stack that did not behave as expected. These kinds of issues tend to be time-consuming in a way that is hard to plan for because they are often invisible until you are already deep into an implementation. The two that hurt the most were user authentication and the jobs tracking system. We had estimated authentication at about four weeks of dev time, but we ended up getting it done faster than that because bcrypt @bcrypt handled most of the hard parts. The jobs system, however, went the other direction entirely, where what looked like a straightforward progres-tracking feature turned out to require a whole new subsystem to even make tractable.

...
The other thing that bothered us about existing tools is how they all handle organization. Folder hierarchies are the default everywhere, which works fine until your library gets large or complex enough that a single image reasonably belongs in multiple categories at once. Google Photos has this existing folder structure, and they also have a unique approach with Gemini @gemini AI-powered search, but it still does not give you a way to define your own categorical labels in any structured way. DigiKam technically has support for tags, but they are clearly secondary to the folder model. Apple Photos has albums, which are basically just folders with a nicer name. None of these tools treat user-defined granular organization as a primary feature, which means (as an example) a photographer who wants to label a batch of images by client, usage rights, subject, and location all at once, and then later query across any combination of those, has no good way to do it.
...
#figure(
  image("../assets/System Architecture.png", width: 85%),
...
#figure(
  ...
  caption: [Gallery Page showing the Tags dropdown open],
...
#figure(
  image("../assets/Gallery with Sort Dropdown.png", width: 92%),
  caption: [Gallery Page showing the Sort dropdown open],
...
#figure(
  image("../assets/PreviewWindow.png", width: 80%),
  caption: [Preview Window Panel],
) <preview_window>
...
#figure(
  image("../assets/Settings Page.png", width: 92%),
...
#figure(
  image("../assets/CustomThemeCreator.png", width: 79%),
  caption: [Custom Theme Creation Feature],
) <custom_theme_feature>
...
I would also have started active development earlier in the Fall 2025 semester. A significant portion of that semester was spent on planning, designing, and evaluating options, which was valuable but could have been compressed given how clear the project vision was from the start. Starting earlier would not necessarily have changed what we built, but it likely would have given us more time to add features and polish in the final stretch.
